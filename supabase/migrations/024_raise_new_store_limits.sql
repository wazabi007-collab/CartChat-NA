-- Raise new-store trust value limit from N$5,000 → N$10,000 (order count stays at 10)
-- Also backdate Octovia Nexus store to bypass new-store check (test store)

-- 1. Backdate Octovia Nexus store so it's no longer "new" (bypasses the 30-day check)
UPDATE merchants
SET created_at = now() - interval '90 days'
WHERE id = '6c49bff7-0000-0000-0000-000000000000'
   OR store_name = 'Octovia Nexus';

-- 2. Recreate place_order with raised limits
...
BEGIN
  ...
  -- Limits: 10 orders/month, N$10,000 total value/month
