...
/**
 * Anchor for section N. The contents list and the rendered heading both go
 * through this, so a contents link can never point at an id nothing renders.
 */
const sectionAnchor = (n: number) => `step-${n}`;
...
function Section({
  ...
  return (
    <section id={sectionAnchor(n)} className="scroll-mt-20">
...
/**
 * The guide's single source of truth. Section numbers, the contents list and
 * the anchors are all derived from this array's order, so inserting a section
 * in the middle can no longer leave the contents list naming the wrong
 * subjects. Keep in step with scripts/merchant-guide.html, which prints the
...
const SECTIONS: GuideSection[] = [
  {
    ...
    body: (
      <Note>
        You do not need a website, a domain, or a card machine. Your store link{" "}
        <em>is</em> your website.
      </Note>
    ),
  },
  {
    ...
    body: (
      <Steps>
        <Step n={1}>
          Sign up, then open the setup wizard. It starts automatically the first
          time.
        </Step>
        <Step n={2}>
          Enter your <strong className="text-walnut">store name</strong> — this
          is what customers see, so use your real trading name.
        </Step>
        <Step n={3}>
          Enter your <strong className="text-walnut">WhatsApp number</strong>.
          Orders and alerts come to this number, so use the phone you actually
          carry.
        </Step>
        <Step n={4}>
          Pick your <strong className="text-walnut">region and town</strong>.
          Customers filter by town when browsing, so this decides who finds you.
        </Step>
        <Step n={5}>
          Choose your <strong className="text-walnut">industry</strong>. It sets
          your store&apos;s layout and category icons — a bakery looks different
    ...
    body: (
      <>
        <Steps>
          ...
          <Step n={3}>
            Add a photo. Natural daylight, plain background, product filling the
            frame. Photos sell more than words do.
          </Step>
          <Step n={4}>
    ...
    body: (
      <>
        <Steps>
          ...
          <Step n={2}>
            Tick the methods you accept: bank transfer (EFT), cash on delivery
            or collection, MTC Maris, eWallet, FNB Pay2Cell, or PayToday.
          </Step>
          <Step n={3}>
  ...
  {
    title: "Selling rules",
    lead: "To keep OshiCart safe and trusted, some items may not be sold here.",
    body: (
      <>
        <p className="mt-4 leading-relaxed text-walnut-2">
          These are <strong className="text-walnut">prohibited</strong>: adult
          content, illegal drugs, counterfeit goods, fraud, and dangerous or
          restricted items. Stores that break the rules can be hidden or
          suspended.
        </p>
        <Note>
          Read the full list before you list:{" "}
          <Link
            href="/prohibited-products"
            className="font-bold text-terracotta hover:underline"
          >
            oshicart.com/prohibited-products
          </Link>
          .
        </Note>
      </>
