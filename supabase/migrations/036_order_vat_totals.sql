-- ============================================================
-- 036 - Order VAT totals
--
-- Stores a VAT snapshot on each order and makes place_order compute
-- VAT server-side from the merchant's current VAT settings. This keeps
-- checkout, order notifications, dashboards, and invoices aligned.
-- ============================================================
...
SET vat_inclusive = false
WHERE COALESCE(TRIM(vat_number), '') <> '';

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS vat_nad integer NOT NULL DEFAULT 0,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'orders_vat_nad_non_negative'
  ) THEN
    ALTER TABLE public.orders
      ADD CONSTRAINT orders_vat_nad_non_negative CHECK (vat_nad >= 0);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'orders_vat_rate_bps_valid'
  ) THEN
    ALTER TABLE public.orders
      ADD CONSTRAINT orders_vat_rate_bps_valid CHECK (vat_rate_bps >= 0 AND vat_rate_bps <= 10000);
  END IF;
END $$;
...
BEGIN
  SELECT
    ...
    m.delivery_fee_nad,
    m.vat_number,
    COALESCE(m.vat_inclusive, false) AS vat_inclusive
  INTO v_merchant
  FROM merchants m
    ...
    SELECT
      ...
      COALESCE(
        SUM(
          ...
          + CASE WHEN vat_inclusive THEN 0 ELSE COALESCE(vat_nad, 0) END
        ),
        0
      )
